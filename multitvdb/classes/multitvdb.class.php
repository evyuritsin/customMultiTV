...
class multiTvDbClass
{
    ...
    //проверка и вывод сообщений и наличии таблицы и необходимых полей
    public static function checkConfigFile($field_id) {
        $tvName = self::getTVName($field_id);
        $filename = MULTITVDB_BASE_PATH . 'configs/'. $tvName. '.config.php';
        if (file_exists($filename)) {
            return 'true';
        } else {
            return 'Файл конфигурации tv поля отсутствует.';
        }
    }
    //проверка и вывод сообщений и наличии таблицы и необходимых полей
    public static function checkTableDB($tablename, $fields ) {
        global $modx;
        $table = $modx->getFullTableName($tablename);
        $fields[] = 'id';
        $fields_list = implode(',', $fields);
        $checkTable = $modx->db->query( "SHOW TABLES LIKE '". $modx->getFullTableName($tablename). "'" );
        if( $modx->db->getRecordCount( $checkTable ) >= 1 ) {
            $data = $modx->db->select('*', $modx->getFullTableName($tablename));
            if( $modx->db->getRecordCount( $data ) >= 1 ) {
                while( $row = $modx->db->getRow( $data ) ) {
                    foreach ($fields as $field) {
                        if (!isset($row[$field])) $notfield[] = $field;
                    }
                    if (isset($notfield)) {
                        $fieldlist = implode(',', $notfield);
                        return 'В таблице "'. $modx->getFullTableName($tablename). '" отсутствуют следующие поля - '. $fieldlist;
                    } else {
                        return 'true';
                    }                    
                }
            ...
            return 'Таблица "'. $table. '" не существует.';
        }
    }    
